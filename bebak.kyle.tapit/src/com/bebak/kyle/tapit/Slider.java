...
int topcornerx, topcornery; // location of slider
  ...
  /** Constructor **/
  Slider (TapIt tapIt, int topcornerx, int topcornery, int dimx, int dimy, 
  Item[] values, int initialIndex, String slidername) {
    ...
    ticks = values.length;
    tickval = TapIt.constrain(initialIndex, 0, values.length - 1);

    dim = TapIt.max(dimx, dimy);
    ...
    if (dimx >= dimy) {
      ...
      xbutton = topcornerx + TapIt.round( dim * tickval / (float) values.length);
      ...
      ybutton = topcornery + TapIt.round( dim * tickval / (float) values.length);
    }
  ...
  public void update() {
    ...
    if (toggleMove) {
      ...
      if (dimx >= dimy) {
        tickval = TapIt.constrain(TapIt.floor((parent.mouseX - topcornerx) / tickspacingclick), 0, ticks - 1);
        xbutton = topcornerx + tickval * tickspacing;
      }
      else {
        tickval = TapIt.constrain(TapIt.floor((parent.mouseY - topcornery) / tickspacingclick), 0, ticks - 1);
        ybutton = topcornery + tickval * tickspacing;
      }
    }
  }
