...
private String path;
  ...
  public Scores(MatchIt matchIt, int spc, String mode) {
    sketch = matchIt;
    ...
    for (Integer i : MatchIt.spcNumbers) {
      mrcIndices.put(MatchIt.sdPath + "/Survivor" + "/" + Integer.toString(i) + ".txt", -1);
      mrcIndices.put(MatchIt.sdPath + "/TimeTrial" + "/" + Integer.toString(i) + ".txt", -1);
    }
 ...
 * @param matchIt TODO************/
  ...
  private void buildPath(int spc, String mode) {
    path = new String();
    if (mode.equals(MatchIt.SURVIVOR))
      path = MatchIt.sdPath + "/Survivor" + "/" + spc + ".txt";
    if (mode.equals(MatchIt.TIME_TRIAL))
      path = MatchIt.sdPath + "/TimeTrial" + "/" + spc + ".txt";
  }
  ...
  public void display() {
    ...
    sketch.textAlign(MatchIt.LEFT, MatchIt.BASELINE); // return text alignment to default setting
  }
  ...
  public void addScore(int score, int spc, String mode) {
    ...
    for (int i = 0; i < scores.length; i++) {
      ...
      if (mode.equals(MatchIt.SURVIVOR) && score > Integer.parseInt(scores[i])) {
        // score in text file must be converted to an integer to allow comparison
        mrcIndex = i;
        break;
      }

      if (mode.equals(MatchIt.TIME_TRIAL) && score < Integer.parseInt(scores[i])) {
  ...
  public int checkClick() {
    if (!inErase) {
      if (MatchIt.dist(sketch.mouseX, sketch.mouseY, eraseX, eraseY) < eraseTextSize * 1.25f)
        return 1;
      return 0;
    } 
    else {
      if (MatchIt.dist(sketch.mouseX, sketch.mouseY, noX, noY) < eraseTextSize * 1.25f)
        return 2;
      if (MatchIt.dist(sketch.mouseX, sketch.mouseY, yesX, yesY) < eraseTextSize * 1.25f)
        return 3;
    }
  ...
  public void createDirectory(String directory) {
    try {
      ...
      if (!file.exists()) {
        file.mkdirs();
        MatchIt.println("directory created : " + directory);
      }
    }
    catch(Exception e) { 
      e.printStackTrace();
  ...
  public void createFile(String fileName, String[] lines) {
    try {
      ...
      if (!file.exists()) {
        ...
        MatchIt.println("file created : " + fileName);
      }
    }
    catch(Exception e) { 
      e.printStackTrace();
  ...
  public void overwriteFile(String fileName, String[] lines) {
    try {
      sketch.saveStrings(fileName, lines);
      MatchIt.println("file overwritten : " + fileName);
    }
    catch(Exception e) { 
      e.printStackTrace();
    }
